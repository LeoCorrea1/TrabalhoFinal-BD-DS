...
public class ReservaDAO {
    ...
    public boolean existeReservaAtivaOuValida(int idExemplar){
        String sql="""
            SELECT COUNT(*) FROM Reserva
             WHERE id_exemplar=? AND status='ativa' AND data_expiracao>NOW()
        """;
        try(Connection c=Db.getConnection();
            PreparedStatement ps=c.prepareStatement(sql)){
    ...
    public boolean insert(Reserva r){
        ...
        try(Connection c=Db.getConnection();
            ...
            if(rows>0){
                try(ResultSet g=ps.getGeneratedKeys()){if(g.next())r.setIdReserva(g.getInt(1));}
    ...
    public boolean update(Reserva r){
        String sql="""
            UPDATE Reserva SET id_exemplar=?,id_usuario=?,data_reserva=?,data_expiracao=?,status=? 
             WHERE id_reserva=?
        """;
        try(Connection c=Db.getConnection();
            PreparedStatement ps=c.prepareStatement(sql)){
            ...
            if(ok&&("cancelada".equalsIgnoreCase(r.getStatus())||"expirada".equalsIgnoreCase(r.getStatus()))){
                ...
                new MovimentacaoDAO().registrar(null,r.getIdExemplar(),r.getIdUsuario(),
                        "reserva_cancelada",
                        "Reserva cancelada ou expirada para usuário ID="+r.getIdUsuario()+
                                ", exemplar ID="+r.getIdExemplar());
            }
            return ok;
        }catch(SQLException ex){ex.printStackTrace();}
        return false;
    ...
    public boolean delete(int idReserva,int idExemplar){
        ...
        try(Connection c=Db.getConnection();
            ...
            boolean ok=ps.executeUpdate()>0;
            if(ok){
                atualizarDisponibilidadeExemplar(idExemplar,true);
                new MovimentacaoDAO().registrar(null,idExemplar,null,
                        "exclusao","Registro excluído: tipo=Reserva id="+idReserva);
            }
            return ok;
        }catch(SQLException ex){ex.printStackTrace();}
        return false;
    ...
    private void atualizarDisponibilidadeExemplar(int idExemplar,boolean disp){
